...
description: Scelte di packaging dell'app desktop Electron — frontend Next.js come export statico, electron-builder, formati di installazione per OS, firma e auto-update.
...
Per Magistra la differenza è dirimente, perché il backend non è un guscio sottile ma esegue logica Node in-process:
...
## Packaging del frontend Next.js: export statico

Il [frontend Next.js](./frontend.md) viene impacchettato come **export statico** (`output: 'export'`): HTML, JS e CSS pre-renderizzati, inclusi nel bundle e caricati nel renderer di Electron tramite un **protocollo applicativo locale** (es. `app://`), senza alcun server.
...
- di conseguenza Magistra **non usa** SSR né le API route di Next.js: nessuna parte server-side del framework gira a runtime.

L'alternativa — `output: 'standalone'`, cioè un **server Node di Next.js embedded** nel processo principale — è **scartata**: aprirebbe una porta HTTP su `localhost`, aumentando la superficie d'attacco (vedi [sicurezza](../requisiti/sicurezza.md)) e duplicando un trasporto di rete dove l'IPC è già sufficiente per un'app **single-utente** locale.
L'export statico mantiene il frontend un puro insieme di asset: il renderer mostra la UI, ogni logica passa per l'IPC verso il backend.

## Strumento di build: electron-builder

Il packaging usa **electron-builder** con **electron-updater**.
